...
export default function PrintPage() {
  ...
  const getGroupMatches = (teams: any[]) => {
    ...
    <div className="min-h-screen bg-slate-50 text-slate-900 p-4 md:p-8 font-sans print:bg-white print:p-0">
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 mb-6">
          ...
          <div className="flex items-center gap-4">
            ...
            <button
              ...
              className="px-6 py-3 rounded-xl font-bold text-white bg-blue-600 hover:bg-blue-700 transition-colors shadow-[0_10px_20px_rgba(37,99,235,0.2)] flex items-center gap-2 group"
            >
              <svg
        ...
        <div className="flex flex-wrap gap-2 pt-6 border-t border-slate-100">
          {Object.entries(tabLabels).map(([key, label]) => (
            <button
              ...
              className={`px-5 py-2.5 rounded-xl font-bold text-sm tracking-wide uppercase transition-all ${
                activeTab === key
        ...
        {activeTab === "groups" ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 print:grid-cols-2 gap-x-8 gap-y-8 print:gap-x-6 print:gap-y-6">
            {groups.map((group, idx) => {
              const groupMatches = getGroupMatches(group.teams);
              return (
                <div
                  key={idx}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 print:grid-cols-2 gap-x-8 gap-y-6 print:gap-x-6 print:gap-y-4">
            {Array.from({ length: knockoutCounts[activeTab] }).map((_, i) => (
              <div
                key={i}
      ...
      <style
        dangerouslySetInnerHTML={{
          __html: `
        @media print {
          /* Optimized A4 page margins */
          @page { margin: 1cm; size: A4 portrait; }
          
          /* Override pt-16 (header padding) in the global layout */
          body { 
          ...
          /* WE'RE ELIMINATING THE GLOBAL DARK NAVBAR! */
          nav { display: none !important; }
          
          /* We hold the group boxes together */
