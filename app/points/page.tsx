...
export default function MyPointsClientPage() {
  ...
  useEffect(() => {
    async function loadData() {
      ...
      const { data: predData } = await supabase.from("predictions").select(`
          ...
          matches (id, home_team, away_team, home_code, away_code, kickoff_at, status, home_score, away_score, group_name)
        `);

      if (predData) {
        const formatted = (predData as any[]).sort((a, b) => {
          return (
            new Date(b.matches.kickoff_at).getTime() -
            new Date(a.matches.kickoff_at).getTime()
          );
        });
        setAllPredictions(formatted);
      }
      setLoading(false);
    }
    loadData();
  ...
  const groupedHistory = filteredHistory.reduce(
    (groups, item) => {
      const phase = item.matches.group_name || "Other";
  ...
  return (
    <div className="max-w-[1200px] mx-auto px-4 md:px-8 pt-28 pb-12 overflow-hidden">
      <Link
        href="/"
          ...
          {Object.entries(groupedHistory).map(([phase, items]) => (
            <div key={phase} className="flex flex-col gap-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {items.map((item, idx) => {
                  // Anti-Cheat: If not my profile then the tipps are not showed
                  const isSecret =
